...
// Supondo que você tem uma estrutura MenorCaminho e Aresta definida em algum lugar

int caminhoJaEncontrado(ListaCaminhos* encontrados, MenorCaminho caminhoAtual) {
    for (int i = 0; i < encontrados->numeroDeCaminhos; i++) {
        if (compararCaminhos(&encontrados->caminhos[i], &caminhoAtual)) {
            return 1;
        }
    }
    return 0;
...
void adicionarCaminhoEncontrado(ListaCaminhos* encontrados, MenorCaminho caminhoAtual) {
    encontrados->caminhos = realloc(encontrados->caminhos, (encontrados->numeroDeCaminhos + 1) * sizeof(MenorCaminho));
    encontrados->caminhos[encontrados->numeroDeCaminhos] = caminhoAtual;  // Assumindo uma atribuição superficial aqui; pode ser necessário copiar membros profundamente
    encontrados->numeroDeCaminhos++;
}
...
Aresta* encontrarArestasRemover(Grafo* copiaGrafo, MenorCaminho caminhoAtual) {
    // Lista para armazenar as arestas a serem removidas
    Aresta* arestasParaRemover = NULL;

    // Percorrer o caminhoAtual do destino até a origem
    int noAtual = caminhoAtual.destino;
    while (noAtual != caminhoAtual.origem) {
        int noAnterior = caminhoAtual.noAnterior[noAtual];
        
        // Encontrar a aresta que liga noAnterior a noAtual
        Aresta* aresta = copiaGrafo->vertices[noAnterior].proxima;
        while (aresta != NULL) {
            if (aresta->destino == noAtual) {
                // Esta aresta faz parte do caminho e deve ser removida
                
                // Criar uma nova Aresta para adicionar à lista de arestasParaRemover
                Aresta* novaAresta = malloc(sizeof(Aresta));
                novaAresta->origem = noAnterior;
                novaAresta->destino = noAtual;
                novaAresta->peso = aresta->peso; // Copiar o peso se necessário para sua lógica
                novaAresta->proxima = arestasParaRemover;
                arestasParaRemover = novaAresta;

                break; // Parar a busca pois a aresta foi encontrada
            }
            aresta = aresta->proxima;
        }

        // Mover para o próximo nó no caminho
        noAtual = noAnterior;
    }

    return arestasParaRemover;
}


void liberarArestas(Aresta* arestasRemover) {
    // Percorra a lista ligada (ou estrutura de dados) de arestas e libere a memória
    Aresta* atual = arestasRemover;
    while (atual != NULL) {
        Aresta* temp = atual;
        atual = atual->proxima; // Assumindo que 'proxima' é o ponteiro para a próxima aresta na lista ligada
        free(temp);
    }
}
...
MenorCaminho *yen(Grafo *grafo, int origem, int destino, int k)
{
    MenorCaminho *caminhos = malloc(k * sizeof(MenorCaminho));
    // Inicializar todos os custos como infinito ou um valor inválido para verificar depois
    for (int i = 0; i < k; i++)
    {
    ...
    ListaCaminhos encontrados;

    for (int i = 0; i < k; i++)
    {
        Grafo copiaGrafo;
        copiarGrafo(grafo, &copiaGrafo);

        dijkstra(&copiaGrafo, origem);

        MenorCaminho caminhoAtual = encontrarCaminho(&copiaGrafo, origem, destino);

        // Verifique se o caminho atual já foi encontrado anteriormente
        if (caminhoAtual.custo != INT_MAX && !caminhoJaEncontrado(&encontrados, caminhoAtual))
        {
            caminhos[i] = caminhoAtual;
            adicionarCaminhoEncontrado(&encontrados, caminhoAtual);
        }
        else
        {
            // Se o caminho já foi encontrado ou é inválido, tente a próxima iteração
            liberarCaminho(&caminhoAtual);
            continue;
        }

        // Encontre as arestas a serem removidas e modifique o grafo
        Aresta *arestasRemover = encontrarArestasRemover(&copiaGrafo, caminhoAtual);

        for (Aresta *a = arestasRemover; a != NULL; a = a->prox)
        {
            removerAresta(&copiaGrafo, a->origem, a->destino);
        }

        // Restaure o grafo original para a próxima iteração
        liberarArestas(arestasRemover);
        restaurarGrafo(&copiaGrafo, grafo);
    }
